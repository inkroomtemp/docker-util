...
ADD . /go
WORKDIR /go/cla
RUN cd /go/cla/ && rm -rf clash && git clone https://github.com/Kr328/clash.git clash && go env -w GO111MODULE=on && go env -w GOPROXY=https://goproxy.cn,direct && go get && go build -ldflags="-s -w" main.go && chmod +x main
